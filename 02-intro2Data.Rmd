...
<span style="color: Blue;font-size:22px;">   Motivating scenario:  </span>  <span style="color: Black;font-size:18px;"> We start at the beginning a study -- how do we set up a good way to keep track of data that will allow us to focus on understanding and analyzing our data, not reorganizing and rearranging it? </span> 
...
```{r collecting, fig.cap='Intro to this chapter (2 min and 45 sec).', echo=FALSE, out.extra= 'allowfullscreen'}
include_url("https://www.youtube.com/embed/TPWPzrzY_1M")
```
...
```{r, echo = FALSE, echo=FALSE, out.extra= 'allowfullscreen'}
include_url("https://www.youtube.com/embed/0f8PTAQGdGY")
```
...
You should refer to thing in the same thoughtful way throughout a column. Take, for example, gender as a nominal variable.
...
What is a **data dictionary**, you ask? All the TMI that you would otherwise put in you overly complex column names. So, each row should correspond to a variable in the main data file, and each column should be additional info. For example, columns could be `name`, `description`, `type`, `unit`,`range_or_values`, etc...  
...
`r ji(keyword = "thinking")` Why? `r ji(keyword = "thinking")` These  extra markings will either be lost or add an extra degrees of difficultly to your analysis. Reserve such markings for the presentation of data. 
...
```{r modetidy, fig.cap='More about tidy data (3 min and 30 sec).', echo=FALSE, out.extra= 'allowfullscreen'}
include_url("https://www.youtube.com/embed/pYNEpBTmfoI")
```
...
```{block2, type='rmdtip'}
If you want to be super cool and get the most out of `R`, I suggest starting an [`R` project](https://r4ds.had.co.nz/workflow-projects.html) for each project folder. This is great practice, but might not be appropriate at the intro level. 
```
...
The assignment is on canvas.  YB ADD LINK. It consists of three parts   
...
3. Quiz questions that are the same as those below. If you try them first here, you'll be sure to enter the right answers on canvas.  
...
```{r, echo=FALSE}
knitr::include_app('https://brandvain.shinyapps.io/02-data/', height = '500px')
```
