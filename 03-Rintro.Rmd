...
Using RStudio Cloud allows you to use R and RStudio without installing anything on your computer. Additionally, if you log into our course project on RStudio Cloud you will not need to install any packages or load any files except those you want for fun or for your independent project at the end of term. 
...
- Using the course project guarantees that you’ll get the same environment as the prof and TA, making it easier for them to help you.   

####  Disadvantages to RStudio Cloud {-}  

- **The biggest disadvantage of RStudio cloud is that it's expensive if we all use it and I only paid for a certain amount of usage, so please use it only if you're having issues using R on your computer.** Please e-mail   me f you would like to use it!   
...
- Using the course project on RStudio Cloud might prevent you from learning how to load data, download packages etc.  
...
- 'Is four divided by four greater than or equal to one?' like this `4/4 >= 1` (note an greater then sign, then an equals sign), which returns `TRUE`.   
...
With these variables assigned, we can do simple math, ask logical questions , use them as arguments for functions. For example `sqrt(x = num_vec)` returns `2, 3, 4`. <span style="color: lightgrey;">   Note, this is the same as typing `sqrt(num_vec)` and with just one argument (`x`) for the function, this is totally unnecessary. When there are more arguments it is good practice to include the argument name and the equals sign, as this makes code easier to read, and more likely to work (otherwise R assumes we put things in a specific order).</span>
...
- `numeric` to `factor` -- **DON'T DO THIS, UNEXPECTEDLY BAD THINGS WILL HAPPEN**.  
...
```{r, echo=FALSE, out.width='60%'}
include_graphics("images/loadtidyverse.gif")
```
...
Follow these `R`ules to help make this class and your life easier. 

### Today!   {-}   

1. **Make a folder** for this course and include a subfolder called data (as I said in the last chapter).   

```{r, echo=FALSE,out.width="60%"}
include_graphics("images/applied_biostats_folder.jpeg")
```

2.  Open `R` right now and make a *project* called `applied_biostats` in the existing folder, `applied_bioststats`.   

```{r,echo=FALSE,out.width="60%"}
include_graphics("images/makingaproject.jpeg")
```

### Everyday for class homework etc!   {-}   


1. Double click the `applied_biostats.Rproj` to open RStudio, rather than opening RStudio directly (or if you insist, open the project after you open RStudio).   

2. Open a new script.   

3. Write the date and what we are working on  in a comment (that is, after a hash `#`)

4. Load all packages you will need (almost always tidyverse, sometimes more)

5. Before doing anything else, save the script in your applied_biostats folder with a nice name (e.g. `class_02_intro_to_R.R`).  
...
Teaching this class for years, I've learned a few things about leaning R that you should know.  
...
Today at the RStudio conference, Hadley Whickham, conrasted the "automatic negative thoughts" we feel some time in coding to the "balanced alternative" that we want to get to.  

```{r,echo=FALSE, out.width = "60%"}
include_graphics("images/automaticVbalanced.jpeg")
```
...
Complete the quiz below, a nearly  identical quiz will be on [canvas](https://canvas.umn.edu/courses/225135/quizzes/410138).

```{r, echo=FALSE}
include_app("https://brandvain.shinyapps.io/intro2r/",height = '800')
```
