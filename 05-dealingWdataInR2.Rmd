...
<span style="color: Red;font-size:40px;">   In progress not ready for class yet </span> 
...
- Watch the one required videos from Stat 545 -- one about using [dplyr](https://www.youtube.com/watch?v=Gms3zSPqCk8) and one optional one about [tidying data](https://www.youtube.com/embed/qivE6exNsZI) embeded in the text (feel free to skip the videos featuring me),  and
...
We can also conduct operations by value of a variable. Say we wanted to know each specie's smallest BMI. We would want to [`group_by()`](https://dplyr.tidyverse.org/reference/group_by.html) species. So, for example,  we could type:  
...
**Be sure to ungroup after  you're done or a group will stick together.** 
...
```{r tidying, fig.cap='Tidying data (first 5 min and 40 seconds are required).',  echo=FALSE, out.extra= 'allowfullscreen'}
