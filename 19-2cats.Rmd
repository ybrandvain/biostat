...
moderna_perm_for_chi2 <- bind_rows(select( moderna_perm_summaries, treatment, covid, replicate, count) , 
          select( moderna_perm_summaries, treatment, covid, replicate, count) %>%
...
covid_chisq_test <- covid_counts      %>%
  select(- outcome)                 %>%  # to ignore labels
...
cows_exact_test <- cows   %>%
  select(- bitten)         %>%  # to ignore labels
  fisher.test()
