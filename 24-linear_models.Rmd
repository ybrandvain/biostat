...
range_shift_lm %>%
  augment()    %>%
  select(elevationalRangeShift, .fitted, .resid)
```
...
lizards <- read_csv("https://whitlockschluter3e.zoology.ubc.ca/Data/chapter12/chap12e3HornedLizards.csv")  %>% na.omit()
lizards <- rep_sample_n(lizards , size= nrow(lizards)) %>% ungroup() %>% select(-replicate)
ggplot(lizards, aes(x = Survival, y = squamosalHornLength, color = Survival))  +
...
lizard_lm %>% 
  augment() %>%
  select(squamosalHornLength, Survival, .fitted, .resid)
```
...
```{r}
augment(cricket_lm) %>%
  select(lysozyme, spermViability, .fitted, .resid)
```
