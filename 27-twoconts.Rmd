...
augment(snake_lm) %>%
  summarise(ss_total = sum((body_temp - mean(body_temp))^2) ,
            ...
            p_value  = pf(F_val, df1 = df_model, df2 = df_error, lower.tail = FALSE))            %>% select(-  df_model, -df_error)                                                        %>% mutate(p_value  = paste(round(p_value* 10^7,digits = 1),"x10^-7",sep= "")) %>% kable( digits=4)
```
