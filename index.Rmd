...
call.packages <- function(list.packages) {
  ...
  install.packages(pkgs = list.packages[!(list.packages %in% installed.packages())],
...
install.github <- function(pkgs, repos) {
    install.packages(pkgs = c("devtools" %in% installed.packages()),
                   repos = "http://cran.us.r-project.org")
    if(!c(as.character(pkgs) %in% installed.packages())) {
    devtools::install_github(as.character(repos))
  }
}
install.github(pkgs = "tweetrmd",repos = "gadenbuie/tweetrmd")
install.github(pkgs = "emo",repos = "hadley/emo")
install.github(pkgs = "tweetrmd",repos = "gadenbuie/tweetrmd")
install.github(pkgs = "emo",repos = "hadley/emo")
install.github(pkgs = "beyonce",repos = "dill/beyonce")

pkgs.list = c("knitr","tidyverse","blogdown","colorspace",
              "tweetrmd","tufte","emo","here",
              "DT","beyonce","stringr","cowplot",
              "lubridate","ggthemes","ggforce",
              "datasauRus","ggridges","randomNames","infer")
library(tweetrmd)
library(knitr)
...
**Key links:** [Canvas](https://canvas.umn.edu/courses/225135), [Detailed schedule in progress](https://docs.google.com/document/d/1NFDJlFkvB8Lg2w0GS00coKCrjfHYLjGpKkt-GLGdrEU/edit?usp=sharing), [Rough schedule](https://docs.google.com/spreadsheets/d/1oVgknnzJg4SnLZeLCYa8fSH0xdE-uC-6p_jCRghAI7Y/edit#gid=0). 
